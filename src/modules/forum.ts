...
class ForumFLGift implements Feature {
    ...
    private async _init() {
        ...
        forumPosts.forEach((forumPost) => {
			...
			if (postID === "last"){
				postID = (<HTMLElement>forumPost.previousSibling!.previousSibling!).getAttribute("name");
			}
			//create a new element for our feature
			var giftElement = document.createElement("a");
			...
			var giftIconGif = document.createElement("img");
            ...
            async () => {
				...
				if(giftElement.childNodes.length <= 1){
					...
					var postParentNode = giftElement.parentElement!.parentElement!.parentElement!;
					//once at parent node of the post, find the poster's user id
					var userElem = postParentNode.querySelector(`a[href^="/u/"]`);
					//get the URL of the post to add to message
					var postURL = (<HTMLElement>postParentNode.querySelector(`a[href^="/f/t/"]`)!).getAttribute("href");
					//get the name of the current MAM user sending gift
					var sender = document.getElementById("userMenu")!.innerText;
					...
					var url = `https://www.myanonamouse.net/json/bonusBuy.php?spendtype=sendWedge&giftTo=${userName}&message=${sender} wants to thank you for your contribution to the forum topic [url=https://myanonamouse.net${postURL}]${forumTitle}[/url]`;
					//make # URI compatible
					url = url.replace("#","%23");
					...
					if (JSON.parse(jsonResult).success) {
						...
						if (JSON.parse(jsonResult).error === "You can only send a user one wedge per day."){
							giftElement.appendChild(document.createTextNode("Failed: Already Gifted This User Today!"));
						} else if (JSON.parse(jsonResult).error === "Invalid user, this user is not currently accepting wedges") {
