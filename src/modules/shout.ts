...
class GiftNewest implements Feature {
    private _settings: CheckboxSetting = {
        ...
        type: 'checkbox',
        title: 'giftNewest',
        desc: `Places Gift buttons on the Home page for Newest 10 members`,
    };
    private _tar: string = '#sbf';
    ...
    private async _init() {
		...
		Util.trimGiftList();
		let btnMaker: Promise<HTMLElement>;
		//get the FrontPage NewMembers element containing newest 10 members
		let fpNM: HTMLElement = document.getElementById("fpNM")!;
		let members = Array.prototype.slice.call(fpNM.getElementsByTagName("a"));
		for (const member of members) {
			//add a class to the existing element for use in reference in creating buttons
			member.setAttribute("class","mp_refPoint_"+member.innerText);
			//if the member has been gifted through this feature previously
			if(GM_getValue("stor_lastNewGifted").indexOf(member.innerText) >= 0){
				//add checked box to text
				member.innerText = member.innerText+"\u2611";
			} else {
				//add empty box to text
				member.innerText = member.innerText+"\u2610";
			}			
		}
		//get the default value of gifts set in preferences for user page
		...
		let giftAmounts: HTMLInputElement = document.createElement("input");
		giftAmounts.setAttribute("type","text");
		giftAmounts.setAttribute("size","3");
		giftAmounts.setAttribute("id","mp_giftAmounts");
		giftAmounts.setAttribute("title","Value between 5 and 1000");
		giftAmounts.setAttribute("value",giftValueSetting);
		//insert the text box after the last members name
		document.querySelector('.mp_refPoint_'+members[members.length-1].innerText.slice(0,-1))!.after(giftAmounts);
		
		//make the button and insert after the last members name (before the input text)
		await Promise.all([
            (btnMaker = Util.createButton(
                'giftAll',
                'Gift All: ',
                'button',
                '.mp_refPoint_'+members[members.length-1].innerText.slice(0,-1),
                'afterend',
                'mp_btn'
            )),
        ]);
        ...
        btnMaker
            .then((btn) => {
				...
				btn.after(document.createTextNode('\u00A0'));
                btn.addEventListener(
                    'click', async (e) => {
						for (const member of members) {
							...
							if(member.innerText.indexOf("\u2610") >= 0){
								//wait 3 seconds between JSON calls (need to implement an easy way to skip on first call)
								await Util.sleep(3000);
								//get the members name for JSON string
								const userName = member.innerText.replace("\u2610","");
								...
								//URL to GET random search results. *could not figure out how to use ${} to include the parameter variable
								const url = 'https://www.myanonamouse.net/json/bonusBuy.php?spendtype=gift&amount='+giftFinalAmount+'&giftTo='+userName;
								await Promise.all([
								...
								.then((jsonFull) => {
									...
									if(JSON.parse(jsonFull).success){
										//check off box
										member.innerText = member.innerText.replace("\u2610","\u2611");
										//add member to the stored member list
										GM_setValue("stor_lastNewGifted",member.innerText.replace("\u2611","")+","+GM_getValue("stor_lastNewGifted"));	
									}
								})
								.catch((err) => {
									throw new Error(err);
		...
		document.getElementById('mp_giftAmounts')!.addEventListener('input', () => {
                ...
                if (
					...
					(<HTMLInputElement>document.getElementById("mp_giftAll"))!.disabled = true;
					(<HTMLInputElement>document.getElementById("mp_giftAll"))!.setAttribute("title", "Disabled");            
				} else {
					(<HTMLInputElement>document.getElementById("mp_giftAll"))!.disabled = false;
					(<HTMLInputElement>document.getElementById("mp_giftAll"))!.setAttribute("title", "Gift All "+valueToNumber);
                }
            });
        ...
        btnMaker
            .then((btn) => {
				document.querySelector('[id=mp_giftAmounts]')!.after(document.createElement('br'));
				btn.setAttribute("title","Open new tab for each");
				btn.addEventListener(
                    'click',
                    () => {
						for (const member of members) {
							if(member.innerText.indexOf("\u2610") >= 0){
								window.open('https://www.myanonamouse.net'+member.attributes[0].value.replace("\u2610",""), '_blank');
							}
						}
					},
					false
		...
		let messageSpan: HTMLElement = document.createElement("span");
