class ToggleHiddenRequesters implements Feature {
    ...
    constructor() {
        Util.startFeature( this._settings, this._tar, [ 'requests' ] )
    ...
    private _getRequestList ():Promise<NodeListOf<HTMLLIElement>> {
        return new Promise( async (resolve,reject) => {
            ...
            await Check.elemLoad('#torRows .torRow .torRight');
            // Grab all requests
            const reqList:NodeListOf<HTMLLIElement>|null|undefined =
                document.querySelectorAll('#torRows .torRow') as NodeListOf<HTMLLIElement>;

            if(reqList === null || reqList === undefined){
                reject(`reqList is ${reqList}`);
            }else{ resolve(reqList); }
    ...
    private _filterResults (list:NodeListOf<HTMLLIElement>) {
        list.forEach(request => {
            const requester:HTMLAnchorElement|null = request.querySelector('.torRight a');
            if(requester === null){
